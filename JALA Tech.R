...
# create variable baru productivity = biomass/luas kolam

df$productivity <- df$Panen/df$luas

df$productivity <- df$Berat.Panen/df$luas
...
data <- cbind(rep(2,5) , rep(0,5) , tab)
data <- t(data)
...
names(df)
tab <- df[is.na(df$ABW)==F,c(1,3,16)]

tab <- tab %>% 
  select(Age) %>%
  group_by(kolam) %>% 
  summarise_each(funs(max(ABW, na.rm = TRUE)))
